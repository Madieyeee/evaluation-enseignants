    ...
    <div class="space-y-6">
        ...
        <x-ui.card>
            <div class="mb-6">
                ...
                <p class="text-xs text-muted">Definissez un nouveau critere d'evaluation.</p>
            </div>
            <form action="{{ route('admin.criteres.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <x-ui.input type="text" name="nom" :value="old('nom')" placeholder="Ex: Clarte des explications" required />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-foreground mb-1.5">Ordre d'affichage</label>
                        <x-ui.input type="number" name="ordre" :value="old('ordre', 1)" min="1" required />
                    </div>
                </div>
                <div class="mt-5">
                    <label class="block text-sm font-medium text-foreground mb-1.5">Description</label>
                    <textarea name="description" rows="3" class="w-full rounded-lg border border-borderColor/60 bg-surface px-3 py-2 text-sm text-foreground focus:border-accent focus:ring-1 focus:ring-accent transition-colors" placeholder="Decrivez ce critere...">{{ old('description') }}</textarea>
                </div>
                <div class="mt-5">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="est_actif" value="1" class="rounded border-borderColor/60 text-accent focus:ring-accent" checked>
